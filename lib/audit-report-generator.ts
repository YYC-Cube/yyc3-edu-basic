import { AuditResult, AuditIssue, AuditSeverity } from '@/hooks/use-global-audit'
...
export class AuditReportGenerator {
  ...
  private generateSummary(results: AuditResult[]) {
  ...
  private processDimensions(results: AuditResult[]) {
  ...
  private generateImprovements(result: AuditResult): string[] {
    ...
    // 基于问题类型生成改进建议
    const issueTypes = [...new Set(result.issues.map(i => i.rule))]
    
    issueTypes.forEach(ruleType => {
      const issues = result.issues.filter(i => i.rule === ruleType)
      if (issues.length > 0) {
        improvements.push(`解决 ${issues.length} 个 ${ruleType} 问题`)
      }
    })
    ...
    allIssues
      ...
      .forEach((issue, index) => {
        immediate.push({
          ...
          category: this.mapDimensionToCategory(issue.dimension),
          autoApplicable: issue.autoFixAvailable,
...
- **改进建议**: 
${data.improvements.map((imp: string) => `  - ${imp}`).join('\n')}
`).join('\n')}
