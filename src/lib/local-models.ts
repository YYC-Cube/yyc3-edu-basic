...
export interface LMStudioModelData {
  id: string
}
...
const LOCAL_OPENAI_URL = process.env.NEXT_PUBLIC_LOCAL_OPENAI_URL || "http://localhost:5000"
const LOCAL_OPENAI_ALT_URL = process.env.NEXT_PUBLIC_LOCAL_OPENAI_ALT_URL || "http://localhost:5001"

export const LOCAL_MODEL_PROVIDERS: ModelProvider[] = [
  {
    name: "Ollama",
    endpoint: OLLAMA_URL,
    checkEndpoint: `${OLLAMA_URL}/api/tags`,
    models: [],
  },
  {
    name: "LM Studio",
    endpoint: LM_STUDIO_URL,
    checkEndpoint: `${LM_STUDIO_URL}/v1/models`,
    models: [],
  },
  {
    name: "Text Generation WebUI",
    endpoint: LOCAL_OPENAI_URL,
    checkEndpoint: `${LOCAL_OPENAI_URL}/api/v1/models`,
    models: [],
  },
  {
    name: "KoboldCpp",
    endpoint: LOCAL_OPENAI_ALT_URL,
    checkEndpoint: `${LOCAL_OPENAI_ALT_URL}/api/v1/models`,
    models: [],
  },
]
...
export async function fetchAvailableModels(provider: ModelProvider): Promise<LocalModel[]> {
  try {
    ...
    if (provider.name === "Ollama") {
      models =
        data.models?.map((model: OllamaModelData) => ({
          id: model.name,
          name: model.name,
      ...
      models =
        data.data?.map((model: LMStudioModelData) => ({
          id: model.id,
          name: model.id,
...
export async function sendMessageToLocalModel(
  ...
  try {
    let endpoint = ""
    let requestBody: ModelRequestData = {}
